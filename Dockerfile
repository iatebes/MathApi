...
# install dependency
RUN cd /working && apt-get update -y && apt-get install -y software-properties-common && apt-get update -y && add-apt-repository ppa:openjdk-r/ppa -y && apt-get install openjdk-8-jdk -y && apt-get install mysql-server -y && service mysql start && chmod +x init_db_mysql.sh && ./init_db_mysql.sh && mvn -U package && cp -f ./target/MathApi*.war /usr/apache-tomcat-8.5.32/webapps/ROOT.war
